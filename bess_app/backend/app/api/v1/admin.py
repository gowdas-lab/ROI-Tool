...
def get_all_bom_items(db: Session = Depends(get_db)):
    """Get all BOM items from component catalog"""
    return db.query(ComponentCatalog).all()

@router.post("/bom", response_model=ComponentCatalogResponse)
def create_bom_item(item: ComponentCatalogCreate, db: Session = Depends(get_db)):
    """Create new BOM item"""
...
def update_bom_item(item_id: int, item: ComponentCatalogCreate, db: Session = Depends(get_db)):
...
def bulk_import_bom(items: List[ComponentCatalogCreate], db: Session = Depends(get_db)):
...
def bulk_import_suppliers(suppliers: List[SupplierCreate], db: Session = Depends(get_db)):
